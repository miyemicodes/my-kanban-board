  ...
  return (
    <>
      <section className="bg-[#2d2c37] absolute top-10 left-[35%] right-[30%] w-[450px] h-[85vh] p-2 rounded-xl">
        <button onClick={() => handleClick()}  className='p-2 hover:bg-slate-400 rounded-full group'>
        ...
        <form className="flex flex-col justify-start gap-3 px-6 py-3 text-slate-200">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input className="bg-transparent border-2 border-[#403f4e] rounded-md p-2 " 
            value={title}
            onChange={(e) => {
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea className="bg-transparent border-2 border-[#403f4e] rounded-md p-2 " 
            value={description}
            onChange={(e) => {
          ...
          <div className="flex flex-col gap-1">
...

            {
              subTasks.map((subT, index) => (
                <div className="flex items-center justify-between" key={subT.id}>
                  <input className="bg-transparent border-2 border-[#403f4e] rounded-md p-2 w-[90%]" 
                    value={subT.name}
                    onChange={(e) => handleUpdateName(index, e.target.value)}
                  />
                  <button type="button" onClick={() => handleRemoveFormData(index)}>
          ...
          <div className="flex flex-col gap-1">
            ...
            <select className="bg-transparent border-2 border-[#403f4e] rounded-md p-2 outline-none"
             ...
             >
              ...
              {
              store.map(columns => (
                <option key={columns.id} value={columns.id}>{columns.label}</option>
              ))
              }
            </select>
          </div>

          <button className="bg-[#795fc5] text-slate-50 font-semibold py-2 rounded-2xl" type="button" onClick={() => handleSubmit()}>
            Create Task
          </button>
        </form>
      </section>
